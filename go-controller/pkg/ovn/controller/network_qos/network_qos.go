...
func (c *Controller) processNextNQOSWorkItem(wg *sync.WaitGroup) bool {
	...
	if err := c.syncNetworkQoS(nqosKey); err != nil {
		...
		c.updateNQOSStatusToNotReady(nqosNamespace, nqosName, err.Error())
...
func (c *Controller) ensureNetworkQos(nqos *networkqosapi.NetworkQoS) error {
	...
	if len(nqos.Spec.PodSelector.MatchLabels) > 0 || len(nqos.Spec.PodSelector.MatchExpressions) > 0 {
		if podSelector, err := metav1.LabelSelectorAsSelector(&nqos.Spec.PodSelector); err != nil {
			c.updateNQOSStatusToNotReady(nqos.Namespace, nqos.Name, fmt.Sprintf("error parsing source pod selector, %v", err))
			return nil
		} else {
...
func (c *Controller) updateNQOSStatusToNotReady(namespace, name, reason string) {
	...
	klog.Error(msg)
	startTime := time.Now()
	if err := c.updateNQOStatusCondition(cond, namespace, name); err != nil {
